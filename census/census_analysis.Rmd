...
geom_line(aes(group=id), alpha=0.2) + # Cage lines
geom_point(aes(color=treatment), size= 0.2, alpha = 0.2) + # Cage lines
geom_line(data=census_by_treatment, aes(x=date, y=census), size=1) + # Mean lines
geom_point(data=census_by_treatment, aes(x=date, y=census)) + # Mean lines
geom_errorbar(data=census_by_treatment, # Mean lines
...
# Un-comment below to customize plot
# theme(
...
for (group in unique(census_log$treatment)) {
  for (time_frame in list(c("07-19", "08-05"), c("08-05", "08-20"), c("08-20", "09-17"))) {
    data_sub <- census_log %>% filter(date %in% time_frame, treatment == group)
    ...
    stats_time_census <- rbind(stats_time_census, data.frame(treatment=c(group), time_gap=c(time_gap), p=c(p)))
  }
}
